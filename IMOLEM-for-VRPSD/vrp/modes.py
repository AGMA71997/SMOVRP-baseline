    ...
    for iter in range(evo_param.maxiter):
        ...
        for cus in problem.customers:
            for other in problem.customers:
                if cus.id != other.id:
                    cus.generate_actual_tt(evo_param.N, other, problem.travel_times)
...
def lem(evo_param, problem):
    ...
    for iter in range(evo_param.maxiter):
        ...
        if evo_param.no_tree == True and evo_param.spec_init == True and evo_param.spec_inst == True:
            variety = 'NoL'
        print('lem{} iter {} at {}...'.format(variety, iter, problem.problem_name))
...
def dbmoea(evo_param, problem):
    ...
    for iter in range(evo_param.maxiter):
        ...
        for index in range(evo_param.size):
            ...
            if moead.cal_tchbycheff(new_plan, weigh_vectors, index, Z) <= moead.cal_tchbycheff(P[index], weigh_vectors,
                                                                                               index, Z):
                C.append(new_plan)
            else:
                C.append(P[index])
