...
def evo():
    ...
    depot_xy, node_xy, node_demand, time_windows, service_times, travel_times = torch.load(
        data_path + 'problem data ' + str(size),
        map_location=device)
    node_demand = node_demand[problem_index]
    time_windows = time_windows[problem_index]
    service_times = service_times[problem_index]
    travel_times = travel_times[problem_index]
    problem_name = "instance_no_" + str(problem_index)
    problem = vrpclass.Problem(problem_name, size, node_demand, time_windows,
                               service_times, travel_times)

    evo_param = vrpclass.Evo_param(size=size, maxiter=maxiter, N=N, trace=trace, MOmode=MOmode, spec_init=spec_init,
                                   spec_inst=spec_inst, no_tree=no_tree)
...
def fig():
    ...
    if mode == 'allp':
        ...
        # datamaps = ['c101']
        modes = ['lem', 'moea', 'dbmoea', 'lem_DRV_DR', 'lem_DRV_DV', 'lem_DRV_RV', 'lem_DRV_D', 'lem_DRV_R',
                 'lem_DRV_V']
        # modes = ['dbmoea_test']
        ...
        titles = ['Number of vehicles', 'Travel distance', 'Driver remuneration', 'Distance * Remuneration * Number',
                  'Distance * Remuneration']
        save = ['comparison_number', 'comparison_distance', 'comparison_pay', 'comparison_product3',
                'comparison_product2']
        ...
        problem = vrpclass.Problem(map_file, map_type, standard_deviation_file=standard_deviation_file,
...
def table():
    ...
    if mode == 'comparison':
        ...
        datamap = 'rc201'
        filenames = ['result/' + datamap + '/lem.txt', 'result/' + datamap + '/lemV1.txt',
                     'result/' + datamap + '/lemV2.txt', 'result/' + datamap + '/lemV3.txt']
        ...
        datamap = 'rc201'
        filenames = ['result/' + datamap + '/lem.txt', 'result/' + datamap + '/lem_DRV_DR.txt',
                     'result/' + datamap + '/lem_DRV_DV.txt', 'result/' + datamap + '/lem_DRV_RV.txt',
                     'result/' + datamap + '/lem_DRV_D.txt', 'result/' + datamap + '/lem_DRV_R.txt',
                     'result/' + datamap + '/lem_DRV_V.txt']
